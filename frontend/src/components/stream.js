...
const testCert = (server1, server2) => {
  if (1 || server1 === undefined ){
    var server1 = process.env.REACT_APP_WEBSOCKET_URL;
    var server2 = process.env.REACT_APP_WEBSOCKET_URL;
...
const startStreaming = async (e) =>{
    ...
    let serverPri = process.env.REACT_APP_WEBSOCKET_URL;
    if (window.location.protocol == 'http:'){
      var protocol = window.location.protocol.replace('http', 'ws');
    } else {
      var protocol = window.location.protocol.replace('https', 'ws');
