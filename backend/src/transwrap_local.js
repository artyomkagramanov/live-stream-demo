...
// wrapper server
const transwraper = http.createServer(app).listen(port, () => {console.log(`Listening on port: ${port} ${servertype}`)})
...
wsRef.on('connection', (ws, req) => {  
  ...
  console.log(`Got connection, URL: ${req.url}`)    
    ...
    const ffmpeg = callff.spawn(
      'ffmpeg', 
      ['-i', '-', '-vcodec', 'libx264', '-preset', 'veryfast', '-tune', 'zerolatency', '-acodec', 'aac', '-reconnect', '3', '-reconnect_at_eof', '1', '-reconnect_streamed', '3',        '-f', 'flv', rtmpURL
    ]);
